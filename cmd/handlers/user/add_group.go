...
func (s *Service) AddGroup(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if rightsProvider.CheckUserRight(currentUser, lib.AssignUserGroup) {
			err := rightsProvider.AssignUserGroup(requestUserId, requestGroupId)
			if err != nil {
				s.Logger.Errorf("Can't assign group to user %s", err.Error())
				response.InternalServerError()
				return
			}
		}
